...
const resolveOpenAPI31Strategy = async options => {
  ...
  try {
    var _optionsUtil$retrieva;
    // determining BaseURI
    const defaultBaseURI = 'https://smartbear.com/';
    const retrievalURI = (_optionsUtil$retrieva = optionsUtil.retrievalURI(options)) !== null && _optionsUtil$retrieva !== void 0 ? _optionsUtil$retrieva : _empty.url.cwd();
    const baseURI = _empty.url.isHttpUrl(retrievalURI) ? retrievalURI : defaultBaseURI;
